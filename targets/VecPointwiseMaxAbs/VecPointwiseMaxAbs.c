...
PetscErrorCode VecPointwiseMaxAbsAsync_Private(Vec w, Vec x, Vec y,
                                               PetscDeviceContext dctx) {
  PetscFunctionBegin;
  // REVIEW ME: no log event?
  PetscCall(VecPointwiseApply_Private(w, x, y, dctx, 0,
                                      VecAsyncFnName(PointwiseMaxAbs),
                                      w->ops->pointwisemaxabs));
  PetscFunctionReturn(PETSC_SUCCESS);
}
